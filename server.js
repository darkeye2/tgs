...
var server = http.createServer(function(request, response) {
  ...
  console.log("Called resource: " + url.pathname);
  ...
  if (url.pathname == '/status') {
    ...
    response.end(fs.readFileSync("togetherjs/togetherjs.js"));
  } else if (url.pathname.substring(1).startsWith("togetherjs") && url.pathname.endsWith(".js")
            || url.pathname.substring(1).startsWith("public") && url.pathname.endsWith(".js")){
    if (fs.existsSync("togetherjs/" + url.pathname.split("/").pop())) {
      response.writeHead(200, {"Content-Type": "text/javascript"});
      response.end(fs.readFileSync("togetherjs/" + url.pathname.split("/").pop()));
    } else {
       write404(response); 
    }
  } else if (url.pathname.endsWith(".css")){
    if (fs.existsSync(url.pathname.substring(1))) {
      response.writeHead(200, {"Content-Type": "text/css"});
      response.end(fs.readFileSync(url.pathname.substring(1)));
    } else {
       write404(response); 
    }
